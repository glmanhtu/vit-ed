...
from paikin_tal_solver import PY3__cmp__, cmp
from paikin_tal_solver.inter_piece_distance import InterPieceDistance
from paikin_tal_solver.puzzle_importer import PuzzleType
from paikin_tal_solver.puzzle_piece import PuzzlePieceRotation, PuzzlePieceSide
...
class BestBuddyHeapInfo(object):
    ...
    def __cmp__(self, other):
        ...
        return cmp(float(other.mutual_compatibility), float(self.mutual_compatibility))
...
class PuzzleOpenSlot(object):
    ...
    def __init__(self, puzzle_id, coord, piece_id, open_side):
        (row, column) = coord
        self.puzzle_id = puzzle_id
...
class PaikinTalSolver(object):
    ...
    def _check_board_dimensions(self, puzzle_id, location):
        ...
        if actual_dimensions is None:
            ...
            for dim in range(0, len(actual_dimensions)):
    ...
    def _update_open_slots(self, placed_piece):
        ...
        for location_side in location_and_sides:
            ...
            if self._is_slot_open(puzzle_id, location):
                ...
                for bb_id in list(self._best_buddies_pool.values()):
